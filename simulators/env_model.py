...
class Dynamics(nn.Module):
    ...
    def __init__(self, state_dim: int, action_dim: int, hidden_dim: int, sequence_num: int, 
        ...
        self.linear_action = nn.Linear(action_dim, hidden_dim) # to avoid overshadow
    ...
    def forward(self, state, action, hc_in=None, next_state=None, next_reward=None):
        ...
        act = self.linear_action(action) # to avoid overshadow
        x = torch.cat([x, act], dim=1)
        x = self.post(x) # 2*hidden_dim -> hidden_dim
...
class GRU_update(nn.Module):
    ...
    def __init__(self, input_size, state_dim, action_dim, sequence_num, hidden_dim, output_size, num_layers, prediction_horizon, 
        ...
        self.linear_states = nn.Linear(state_dim*sequence_num, hidden_dim // 2)
        self.linear_actions = nn.Linear(action_dim, hidden_dim // 2)
        ...
        self.hx_fc = nn.Linear(2*hidden_dim, hidden_dim)
        self.device = device
    ...
    def forward(self, predicted_values, states, actions, next_state = None, next_reward = None):
        # xy (batch, 1, state_dim+1)
        xy = torch.zeros(size=(states.shape[0], 1, self.output_size)).float().to(self.device)
        # hx: (batch_size, 1, (state_dim+action_dim)*sequence_num)
        #hx = past_time_features.reshape(-1, 1, self.hidden_size)
        # state_dim*sequence_num -> hidden_dim, action_dim -> hidden_dim
        # concat(hidden_dim, hidden_dim)
        ...
        for i in range(actions.shape[1]):
            states_ = self.linear_states(states.flatten(1, -1))
            actions_ = self.linear_actions(actions[:, i, :])
            past_time_features = torch.cat((states_, actions_), dim=-1)
            hx = past_time_features.reshape(1, -1, self.hidden_size) 
...
class SeqModel(nn.Module):
    ...
    def __init__(self, use_gru_update: bool, state_dim: int, action_dim: int, hidden_dim: int, sequence_num: int, future_num: int, 
        ...
        # hidden_dim = 256
        self.gru_nn = GRU_update(state_dim+1, state_dim, action_dim, sequence_num, 512, state_dim+1, 1, future_num, device, train_gamma)
    ...
    def forward(self, state, action, hc_in=None, next_state=None, next_reward=None, detach=False):
        next_state_pred, next_reward_pred, hc, loss1 = self.dynamics_nn(state, action[:, 0], hc_in, next_state, next_reward)
        if detach:
        ...
        if self.use_gru_update:
            ...
            next_state_pred, next_reward_pred, loss2 = self.gru_nn(pred_features, state, action, next_state, next_reward)
...
class MyEnv:
    ...
    def _cal_sensitivity(self, states: ObsType, actions: ActType, next_state: ObsType, reward, K=10, sigma=0.01):
        ...
        #print("noisy_states", noisy_states.shape, "noisy_actions", noisy_actions.shape)
