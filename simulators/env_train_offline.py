...
def main(config: Config):
    ...
    dynamics_lr = str_to_floats(config.dynamics_lr)
    lr_epoch = str_to_ints(config.lr_epochs)
    num_epochs = lr_epoch[-1]
    lr = dynamics_lr[0]
    seq_optimizer = torch.optim.Adam(seqModel.parameters(), lr=lr, weight_decay=config.dynamics_weight_decay)
    print("Learning rate", lr)
    t = trange(num_epochs, desc="Training")
    ...
    for epoch in t:
        ...
        losses1 = np.array([])
        losses2 = np.array([])
        # based on what epochs is, determing the lr
        for i, e in enumerate(lr_epoch):
            if epoch < e:
                if lr != dynamics_lr[i]:
                    lr = dynamics_lr[i]
                    for param_group in seq_optimizer.param_groups:
                        param_group['lr'] = lr                    
        ...
        while update_cnt < config.min_updates_per_epoch:
            ...
            if t_len < config.future_num + 1:
                continue
            # choose between left align, right align or right tail align
            # generate a random number between 0 and 1
            align_choice = np.random.uniform(low=0, high=config.left_align_ratio + config.right_align_ratio + config.right_tail_ratio)
            right_num = 0
            if align_choice < config.left_align_ratio:
                lalign = True
            elif align_choice < config.left_align_ratio + config.right_align_ratio:
                lalign = False
            else:
                lalign = False
                right_num = 15

            if right_num > 0 and t_len < right_num:
                continue
            past_states, past_actions, indices = sample_batch_init(data_train, config.batch_size, config.sequence_num, 
                                                                    tragectory_idx=idx, left_align=lalign, right_num=right_num,
                                                                    device=device)
            if right_num > 0:
                max_steps = right_num - config.future_num
            else:
                max_steps = t_len - config.future_num
            for u in range(max_steps):
                ...
                next_states, next_actions, next_rewards= sample_batch_offline3(data_train,
                                                                ...
                                                                indices=indices,
                                                                device=device)
                # states (batch, sequence, state_dim)
                # action (batch, action_dim)
                states = past_states[:, -config.sequence_num:].to(device)
                #print("actions", past_actions[:, -1:, :].shape, next_actions[:, 0:config.future_num-1, :].shape)
                actions = torch.cat([past_actions[:, -1:, :], next_actions[:, 0:config.future_num-1, :]], dim=1).to(device)
                seqModel.train()
